...
- **PC 仅 macOS**（v2 范围）
- **手动 + 自动上传共存**：自动增量同步与相册工作台手动选择上传并行，每个上传项标记 `source`（auto/manual）
- **队列有限可控**：允许暂停/恢复自动上传、取消手动批次；不允许删除单个队列项、跳过、调序、手动标记完成
- **手动优先排序**：手动上传项优先于自动项（priority DESC, id ASC）；每个文件完成后重新取队首
