...
| 常數                                  | 值                                  | 用途                                                      |
| ------------------------------------- | ----------------------------------- | --------------------------------------------------------- |
| `authReplayWindowSeconds`             | `300`（±5 分鐘）                    | `X-LynavoDrive-Auth-Timestamp` 允許偏差                   |
...
| Header                         | 必填 | 說明                                                                       |
| ------------------------------ | ---- | -------------------------------------------------------------------------- |
| `X-LynavoDrive-File-Key`       | 是   | 唯一檔案 ID                                                                |
| `X-LynavoDrive-Filename-B64`   | 是   | UTF-8 原始檔名的 base64，避免 header 字元問題                              |
| `X-LynavoDrive-Media-Type`     | 是   | `image` / `video`                                                          |
| `X-LynavoDrive-File-Size`      | 是   | 位元組數（decimal string）                                                 |
| `X-LynavoDrive-Auth`           | 是   | HMAC-SHA256 驗證值，不傳 raw pairing token                                 |
| `X-LynavoDrive-Auth-Timestamp` | 是   | Unix seconds，用於 replay window                                           |
| `X-LynavoDrive-Auth-Nonce`     | 是   | client 產生的隨機 nonce（≥128 bits），用於 replay 防護                     |
| `X-LynavoDrive-SHA256`         | 是   | hex-encoded SHA256 of body；簽入 canonical string，body 完整性承諾         |
| `X-LynavoDrive-Created-At`     | 否   | RFC3339                                                                    |
| `X-LynavoDrive-Modified-At`    | 否   | RFC3339                                                                    |
| `X-LynavoDrive-Upload-Mode`    | 否   | 固定 `"background_http"`，僅供 diagnostics/log 使用，不寫入 sidecar schema |
...
- `X-LynavoDrive-Auth = hex(HMAC-SHA256(keyBytes, canonicalString))`。
- `canonicalString` 採固定換行格式，欄位順序固定：
  ...
  **欄位格式鎖死**（避免 mobile/server 組串差異）：
  ...
  - `fileKey`：原樣字串
  - `filenameB64`：與 `X-LynavoDrive-Filename-B64` 完全相同字串
  - `mediaType`：與 `X-LynavoDrive-Media-Type` 完全相同字串
  - `fileSize`：decimal integer，無千分位、無負號
  - `bodySha256Hex`：64 字元小寫 hex
  - `createdAt`：與 `X-LynavoDrive-Created-At` 完全相同字串；**缺省定義**：iOS 側若 optional 為 nil **不送** header，canonical string 用空字串 `""`；若 optional 為空字串 `""` 也**不送** header，canonical string 用 `""`。sidecar 側 `r.Header.Get(...)` 回空字串一律視為缺省，canonical string 用 `""`。**禁止**出現「iOS 送 header 為 `""` 但不納入 canonical」或「iOS 不送 header 卻把 `""` 寫進 canonical」的單邊行為
  - `modifiedAt`：與 `createdAt` 同缺省規則
  - `timestamp`：Unix seconds decimal integer（與 `X-LynavoDrive-Auth-Timestamp` 完全相同字串）
  - `nonce`：原樣字串（與 `X-LynavoDrive-Auth-Nonce` 完全相同字串）

  `bodySha256Hex` 必須與 `X-LynavoDrive-SHA256` 一致；sidecar 在 streaming 過程中計算 body SHA256，結束後與 header 比對，不符回 422 `body_hash_mismatch`。這讓 HMAC 涵蓋 body 承諾，避免 LAN on-path 替換 body。
...
背景 `URLSession` 可能在 `waitingForConnectivity` 中保留 request 很久才真正送出；因此 `X-LynavoDrive-Auth-Timestamp` / nonce 是「建立 task 時」的值，不保證「抵達 sidecar 時」仍在 ±5 分鐘內。這是背景傳輸的正常現象，不代表 pairing token 失效。
...
2. 解析並驗證必要 headers → 缺少則 400；`X-LynavoDrive-File-Key` 必須符合 `^[0-9a-f]{64}$`，不符回 `400 invalid_file_key`
3. 驗證 `X-LynavoDrive-Auth` HMAC、timestamp replay window 與 nonce；不符回 401，並依上方 `status` 分類決定是否對來源 IP 做輕量失敗退避
...
9. **確保 `uploads` row 存在**（新增一個專用 helper，**不用** `UpsertUpload`）：
   ...
   store.EnsureReceivingUpload(store.UploadInit{
       ...
       ClientID:         clientID,
       OriginalFilename: decodedFilename,  // 從 X-LynavoDrive-Filename-B64 decode
...
12. **讀取迴圈結束時立即檢查**（在任何 fsync / Close 之前，避免把錯的 bytes 浪費 fsync I/O）；先 `inactivityTimer.Stop()` 冪等取消後援。
    ...
    **分類優先序鎖死**（必須依此順序判斷，不得任意互換）：
    ...
    4. readErr == nil 且 receivedBytes == fileSize 且 bodySHA256 != X-LynavoDrive-SHA256
...
- `X-LynavoDrive-Upload-Mode` 若不是 `"background_http"`，只寫 diagnostics 並忽略，不影響接收結果
...
```go
// cmd/lynavo-drive-sidecar/main.go
func main() {
    ...
    // 建立單一 gate instance
...
| Header                         | 必填 | 說明                                                                          |
| ------------------------------ | ---- | ----------------------------------------------------------------------------- |
| `X-LynavoDrive-File-Key`       | 是   | 必須等於 path param `{fileKey}`；不一致回 `400 {"status":"filekey_mismatch"}` |
| `X-LynavoDrive-Auth`           | 是   | HMAC over DELETE canonical string                                             |
| `X-LynavoDrive-Auth-Timestamp` | 是   | Unix seconds                                                                  |
| `X-LynavoDrive-Auth-Nonce`     | 是   | Nonce（與 POST 共用同一 LRU）                                                 |
...
**POST 也要加 fileKey 格式驗證**：Phase 1.2 Handler 流程 step 2（解析並驗證必要 headers）補充一項：`X-LynavoDrive-File-Key` 必須符合 `^[0-9a-f]{64}$`，不符回 `400 invalid_file_key`。
...
- 用 `curl` 手動測試（**clientId 必須是已配對的 ASCII-safe 值**；**fileKey 必須是 64 字元小寫 hex**，否則會被 Phase 1.2 step 2 的格式驗證拒絕回 `400 invalid_file_key`）：
  ...
  curl -X POST http://localhost:39394/upload/test-client \
    -H "X-LynavoDrive-File-Key: $FILEKEY" \
    -H "X-LynavoDrive-Filename-B64: dGVzdC5qcGc=" \
    -H "X-LynavoDrive-Media-Type: image" \
    -H "X-LynavoDrive-File-Size: 1024" \
    -H "X-LynavoDrive-SHA256: $BODY_SHA256" \
    -H "X-LynavoDrive-Auth: <hmac-over-canonical>" \
    -H "X-LynavoDrive-Auth-Timestamp: $(date +%s)" \
    -H "X-LynavoDrive-Auth-Nonce: $(openssl rand -hex 16)" \
...
2. **`enqueueNextPendingFileIfIdle(binding:clientId:allowPreparation:) async -> EnqueueResult`** — 背景轉換核心：
   ...
   - 背景 HTTP request 的 `X-LynavoDrive-SHA256` 必須使用 `http_body_sha256`，不得在建立 request 當下臨時計算大檔 hash
...
| HTTP body 提前中斷或內容被替換                                                                | handler 先硬性驗證 `receivedBytes == X-LynavoDrive-File-Size`，再比對 streaming body hash 與必填 `X-LynavoDrive-SHA256`；不符就 `422 file_size_mismatch` / `body_hash_mismatch`，不得 finalize                                                                                                       |
...
| LAN on-path body 替換（HMAC 簽章未涵蓋 body）                                                 | `X-LynavoDrive-SHA256` 改為必填並簽入 canonical string；streaming 中計算 body hash 並 422 拒絕不符內容                                                                                                                                                                                               |
...
2. **Go 編譯**: `go build ./cmd/lynavo-drive-sidecar/`
...
24. **Body hash 綁定測試**: 以正確 HMAC + 不一致的 `X-LynavoDrive-SHA256` / 替換 body 發 POST，sidecar 必須回 422 `body_hash_mismatch`，不得 finalize
...
42. **DELETE path vs header fileKey mismatch 測試**: path `/upload/<cid>/<A>` 但 header `X-LynavoDrive-File-Key: B`，sidecar 必須回 `400 filekey_mismatch`（防止 HMAC 對 A 卻刪 B）
43. **fileKey 格式驗證測試**: POST / DELETE 送 `X-LynavoDrive-File-Key: not-hex-64chars`，sidecar 必須回 `400 invalid_file_key`
...
52. **fileKey owner mismatch security 測試**（必測）：
    ...
    - B 用**自己的** pairingToken 對 path `/upload/<B>/` 發 POST 帶 `X-LynavoDrive-File-Key: F`；sidecar 必須回 `403 file_key_owner_mismatch`，不得覆蓋 A 的 row
...
53. **fileKey 格式驗證跨 endpoint 一致**：POST 和 DELETE 都對 `X-LynavoDrive-File-Key` 做 `^[0-9a-f]{64}$` 檢查；送非 hex / 長度錯誤 / 大寫字元都回 `400 invalid_file_key`
